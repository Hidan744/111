...
# Некоторые мобильные клавиатуры (например, с арабской или персидской
# раскладкой) вставляют цифры не в ASCII-варианте, из-за чего "5" может
# на самом деле прийти как "٥" или "۵" и не совпасть ни с одним id темы.
# Приводим такие цифры к обычным перед сравнением с CATEGORY_BY_ID.
_DIGIT_TRANSLATION = str.maketrans(
    "０１２３４５６７８９"  # полноширинные 0-9
    "٠١٢٣٤٥٦٧٨٩"  # арабские-индийские 0-9
    "۰۱۲۳۴۵۶۷۸۹",  # персидские 0-9
    "0123456789" * 3,
)


def resolve_category_choice(text: str) -> dict | None:
    """
    Сопоставляет введённый пользователем текст с одной из CATEGORIES.

    Помимо "чистой" цифры распознаёт частые причины, из-за которых номер
    темы не находился: цифры в не-ASCII написании (полноширинные,
    арабские, персидские — так присылают некоторые мобильные клавиатуры)
    и случайные пробелы/знаки препинания вокруг цифры (" 5", "5.", "(5)").
    Возвращает саму категорию (dict) или None, если сопоставить не удалось.
    """
    normalized = text.strip().translate(_DIGIT_TRANSLATION).strip(" \t.,()、！!")
